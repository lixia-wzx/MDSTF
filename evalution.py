...
def evaluation(frame_data_result, frame_data_gt, consider_peds):
    ...
    bicycle_final_error = []
    # print(range(0, len(frame_data_result) - predict_len + 1, predict_len))  #range(0, 2485, 6)
    for i in range(0, len(frame_data_result) - predict_len + 1, predict_len):
        current_consider_ped = list(consider_peds[sequence_count])
    ...
    WS_ADE = vehicle_mean_error * vehicle_coe + pedestrian_mean_error * pedestrian_coe + bicycle_mean_error * bicycle_coe
    print('WSADE:', WS_ADE)
    ...
    WS_FDE = vehicle_final_error * vehicle_coe + pedestrian_final_error * pedestrian_coe + bicycle_final_error * bicycle_coe
    print('WSFDE:', WS_FDE)
    print('FDEv, FDEp, FDEb:', vehicle_final_error, pedestrian_final_error, bicycle_final_error)

    return (WS_ADE, vehicle_mean_error, pedestrian_mean_error, bicycle_mean_error,
            WS_FDE, vehicle_final_error, pedestrian_final_error, bicycle_final_error)
...
def evalution_result():
    ...
    parser.add_argument('--gt_dir',
                        default=r'/public/home/aliceyu/anco/wuzixuan/Apolloscape/prediction_gt.txt',
                        help='the dir of ground truth')
    parser.add_argument('--object_file',
                        default=r'/public/home/aliceyu/anco/wuzixuan/Apolloscape/considered_objects.txt',
                        help='the dir of considered objects')
    parser.add_argument('--res_file', default=r'/public/home/aliceyu/anco/wuzixuan/Apolloscape/prediction_result16.txt',
